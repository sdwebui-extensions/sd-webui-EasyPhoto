...
from scripts.easyphoto_utils import (Face_Skin, FIRE_forward, PSGAN_Inference,
                                     ...
                                     decode_base64_to_video, ep_logger,
    ...
    *user_ids,
    ...
    if shared.cmd_opts.just_ui:
        ...
        simple_req = dict(
            ...
            scene_id = scene_id,
            prompt_generate_sd_model_checkpoint = prompt_generate_sd_model_checkpoint,
            ...
            super_resolution_method = super_resolution_method,
            skin_retouching_bool = skin_retouching_bool,
            display_score = display_score,
            background_restore = background_restore,
            background_restore_denoising_strength = background_restore_denoising_strength,
            makeup_transfer = makeup_transfer,
            makeup_transfer_ratio = makeup_transfer_ratio,
            face_shape_match = face_shape_match,
            tabs = tabs,
            ipa_control = ipa_control,
            ipa_weight = ipa_weight,
            ipa_image = None if ipa_image_path is None else api.encode_pil_to_base64(Image.open(ipa_image_path)),
            ref_mode_choose = ref_mode_choose,
            ipa_only_weight = ipa_only_weight,
            ipa_only_image = None if ipa_only_image_path is None else api.encode_pil_to_base64(Image.open(ipa_only_image_path)),
            lcm_accelerate = lcm_accelerate,
            user_ids = user_ids
        )
    ...
    *user_ids,
    ...
    if shared.cmd_opts.just_ui:
        ...
        simple_req = dict(
            webui_id = shared.cmd_opts.uid,
            sd_model_checkpoint = sd_model_checkpoint,
            sd_model_checkpoint_for_animatediff_text2video = sd_model_checkpoint_for_animatediff_text2video,
            sd_model_checkpoint_for_animatediff_image2video = sd_model_checkpoint_for_animatediff_image2video,
            t2v_input_prompt = t2v_input_prompt,
            t2v_input_width = t2v_input_width,
            t2v_input_height = t2v_input_height,
            scene_id = scene_id,
            upload_control_video = upload_control_video,
            upload_control_video_type = upload_control_video_type,
            openpose_video = openpose_video,
            ...
            video_interpolation_ext = video_interpolation_ext,
            tabs = tabs,
            ipa_control = ipa_control,
            ipa_weight = ipa_weight,
            ipa_image_path = ipa_image_path,
            lcm_accelerate = lcm_accelerate,
            user_ids = user_ids
        )

        url = '/'.join([shared.cmd_opts.server_path, 'easyphoto/easyphoto_video_infer_forward'])
        data = requests.post(url, json=simple_req)
        
        outputs = [api.decode_base64_to_image(output) for output in json.loads(data.text)['outputs']]
        output_video = json.loads(data.text)['output_video']
        output_gif = json.loads(data.text)['output_gif']
        comments = json.loads(data.text)['message']
